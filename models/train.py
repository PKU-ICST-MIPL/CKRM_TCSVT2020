...
##############################
use_self_defined_regularization = True
if use_self_defined_regularization:
    weight_decay=0.0001 
...
for epoch_num in range(start_epoch, params['trainer']['num_epochs'] + start_epoch):
    ...
    for b, (time_per_batch, batch) in enumerate(time_batch(train_loader if args.no_tqdm else tqdm(train_loader), reset_every=ARGS_RESET_EVERY)):
        ...
        weight_reg_loss = 0
        if use_self_defined_regularization:
            weight_reg_loss = reg_loss(model)

        loss = output_dict['loss'].mean() + output_dict['cnn_regularization_loss'].mean() + weight_reg_loss
        loss.backward()
        ...
        train_results.append(pd.Series({'loss': output_dict['loss'].mean().item(),
                                        ...
                                        'attention_loss':output_dict['attention_loss'].mean().item(),
                                        'weight_reg_loss':weight_reg_loss,
                                        'accuracy':output_dict['accuracy'].mean().item(),
                                        'accf':output_dict['accf'].mean().item(),
                                        'accg':output_dict['accg'].mean().item(),
